...
bl_dic = {}
bl_dic['effective'] = bl_eff #utils.interp_cl_2d(bl_eff, reso_rad, shape, ell=el_bl, real=False)
bl_dic[95] = bl_95[19:13000] #utils.interp_cl_2d(bl_95, reso_rad, shape, ell=el_bl, real=False)
bl_dic[150] = bl_150[19:13000] #utils.interp_cl_2d(bl_150, reso_rad, shape, ell=el_bl, real=False)
bl_dic[220] = bl_220[19:13000] #utils.interp_cl_2d(bl_220, reso_rad, shape, ell=el_bl, real=False)
...
if read_save_maps:
    ...
    t_95=np.where(hmap_ps_mask<0.1,0.0006,t_95)
    t_150=t_150*hmap_ps_mask
    t_150=np.where(hmap_ps_mask<0.1,7.89E-5,t_150)
    t_220=t_220*hmap_ps_mask
    t_220=np.where(hmap_ps_mask<0.1,-0.00112,t_220)
    ...
    # q_95=np.where(hmap_ps_mask<0.1,-2.16E-5,q_95)
    # q_150=q_150*hmap_ps_mask
    # q_150=np.where(hmap_ps_mask<0.1,4.42E-5,q_150)
    # q_220=q_220*hmap_ps_mask
    # q_220=np.where(hmap_ps_mask<0.1,0.00011,q_220)

    # u_95=u_95*hmap_ps_mask
    # u_95=np.where(hmap_ps_mask<0.1,3.55E-5,u_95)
    # u_150=u_150*hmap_ps_mask
    # u_150=np.where(hmap_ps_mask<0.1,-9.01E-7,u_150)
    # u_220=u_220*hmap_ps_mask
    # u_220=np.where(hmap_ps_mask<0.1,0.00011,u_220)
    ...
    end_time4 = time.time()
    print("CP4 Execution time:", end_time4 - start_time, "seconds")
    ...
    if (0):
        ...
        sys.exit()
    end_time5 = time.time()
    print("CP5 Execution time:", end_time5 - start_time, "seconds")
...
if do_ICL:
    ...
    # nl_oned_dic_QQ={}
    # nl_oned_dic_QQ['QQ']={}
    # nl_oned_dic_QQ['QQ']['95GHz'] = cl_noise_arr_95[1]
    # nl_oned_dic_QQ['QQ']['150GHz'] = cl_noise_arr_150[1]
    # nl_oned_dic_QQ['QQ']['220GHz'] = cl_noise_arr_220[1]
    # nl_twod_dic_QQ = nl_oned_dic_QQ
    # nl_oned_dic_UU={}
    # nl_oned_dic_UU['UU']={}
    # nl_oned_dic_UU['UU']['95GHz'] = cl_noise_arr_95[1]
    # nl_oned_dic_UU['UU']['150GHz'] = cl_noise_arr_150[1]
    # nl_oned_dic_UU['UU']['220GHz'] = cl_noise_arr_220[1]
...
#     nl_twod_dic_BB = nl_oned_dic_BB
    ...
    end_time6 = time.time()
    print("CP6 Execution time:", end_time6 - start_time, "seconds")
    ...
    if full_sky:
        ...
        np.save('./T_95_150_220_winterfield_fullsky_ILC_v2.npy', ilc_map_T)

    end_time7 = time.time()
    print("CP7 Execution time:", end_time7 - start_time, "seconds")
    ...
    ilc_map_T=np.load('./T_95_150_220_winterfield_fullsky_ILC_v2.npy',allow_pickle=True)
    #maps.fitsio.save_skymap_fits('T_95_150_220_winterfield_fullsky_ILC_v2.fits', ilc_map_T, overwrite=True)
    #hp.write_map('T_95_150_220_winterfield_fullsky_ILC_v2_2.fits', ilc_map_T, overwrite=True, partial=True)
