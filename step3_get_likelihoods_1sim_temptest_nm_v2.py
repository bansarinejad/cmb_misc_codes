...
if not add_cluster_tsz:
    ...
    data_stack_dic = {}
    cutouts_rotated_arr=data['clusters']['cutouts_rotated'].squeeze() #Behzad added .squeeze() on 13/03/23 removing dimension 1 (which was left over from when we were doing multiple sims?)
    grad_mag_arr=data['clusters']['grad_mag'].squeeze() #Behzad added .squeeze() on 13/03/23
    ...
    if org_tsz:
        ...
        for i in tqdm(range(25)):
            tsz_estimate_temp = tools.stack_rotated_tqu_cutouts(cutouts_rotated_arr_for_tsz_estimation, weights_for_cutouts=None, perform_random_rotation=True)
            tsz_results.append(tsz_estimate_temp)
        #print(np.asarray(tsz_results).shape)
        tsz_estimate = np.mean(tsz_results, axis=0)
        #print(tsz_estimate)
        #print(np.asarray(tsz_estimate).shape)
    ...
    #print(stack_after_tsz_removal.shape)
    if (0):
        subplot(131); imshow(data['clusters']['stack'][0].squeeze(), cmap=cmap, extent = [x1, x2, x1, x2]); colorbar(); 
...
random_stack = random_stack_dic.squeeze() #[0] Here [0] limits it to T only. Was leftover from sims that doesnt apply to real data. .squeeze() added to remove the undeeded dimension and change shape from (3,1,40,40) to (3,40,40)
...
if (1):
    np.save('./data_25mean_tsz.npy',data)
########################
########################
...
if (1):
    np.save('./data_stack_dic_25mean_tsz.npy',data_stack_dic)
    np.save('./model_dic_25mean_tsz.npy',model_dic)
    np.save('./jk_cov_25mean_tsz.npy',jk_cov)
...
for tqu in range(tqulen):
    ...
    if tqu==0:
        jk_cov_temp=jk_cov[0:1600,0:1600]#[0:400,0:400]
    elif tqu==1:
        jk_cov_temp=jk_cov[1600:3200,1600:3200]#[400:800,400:800]
    elif tqu==2:
        jk_cov_temp=jk_cov[3200:4800,3200:4800]#[800:1200,800:1200]
    loglarr = []
    massarr = []
    ...
    if (recov_mass>=0.0 and recov_mass<=4.0):
...
if plot_qu:
    ...
    for modelcntr, model_keyname in enumerate( sorted( model_dic ) ):
        model_vec_Q = model_dic[model_keyname][1].flatten()
        model_vec_U = model_dic[model_keyname][2].flatten()
