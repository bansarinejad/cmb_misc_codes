...
m_temp=[None]*10
for i in range(10):
  t_temp[i] = np.load('./merged_rand_parts_nw/ILC_cutouts_T_rands_nw_part%s.npy' %(i), allow_pickle= True)
  tp_temp[i] = np.load('./planck/Planck_cutouts_T_nw_masked_rands_WFv3_COMMANDER_part%s.npy' %(i), allow_pickle= True)
  m_temp[i] = np.load('./merged_rand_parts_nw/rands_mask_0p4medwt_6mJy150ghzv2_part%s.npy' %(i), allow_pickle= True)
...
with open('./randoms_m200cut_T_ILC_healpix_nw_apod_zeros_masked_inpaint_planckgrad_WFv3_COMMANDER.pkl','wb') as f:
  pkl.dump(sim_dic, f)
sys.exit()
