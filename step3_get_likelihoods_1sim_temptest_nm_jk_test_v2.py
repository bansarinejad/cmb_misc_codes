...
if (1):
    np.save('./data_jk_test_fitted_tsz.npy',data)
########################
########################
...
if (1):
    np.save('./data_stack_dic_jk_test_fitted_tsz.npy',data_stack_dic)
    np.save('./model_dic_jk_test_fitted_tsz.npy',model_dic)
    np.save('./jk_cov_jk_test_fitted_tsz.npy',jk_cov)
...
if plot_qu:
    ...
    for modelcntr, model_keyname in enumerate( sorted( model_dic ) ):
        ...
        loglval = tools.get_lnlikelihood(data_vec_QU, model_vec_QU, jk_cov_QU) 
        loglarr.append( loglval )
        massarr.append( model_keyname[0] )
    if testing: show(); sys.exit()
    massarr = np.asarray( massarr )
