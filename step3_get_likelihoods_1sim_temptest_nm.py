...
if (1):
    np.save('./data_stack_dic.npy',data_stack_dic)
    np.save('./model_dic.npy',model_dic)
    np.save('./jk_cov.npy',jk_cov)

for tqu in range(tqulen):
    if tqu==0:
        jk_cov_temp=jk_cov[0:400,0:400]
    elif tqu==1:
        jk_cov_temp=jk_cov[400:800,400:800]
    elif tqu==2:
        jk_cov_temp=jk_cov[800:1200,800:1200]
    ...
    for simcntr in sorted(data_stack_dic):
        ...
        if (recov_mass>0.0 and recov_mass<4):
            ...
            plot(massarr_mod, larr, lw = 0.5, label = simcntr);
