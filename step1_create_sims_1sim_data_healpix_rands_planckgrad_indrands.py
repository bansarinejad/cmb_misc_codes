...
for i in range(0,1,1):
  t_temp[i] = np.load('./merged_rand_parts_nw/150GHz_cutouts_T_rands_nw_12arcmin_ps_part%s.npy' %(i), allow_pickle= True)
  tp_temp[i] = np.load('./planck/commander_noINP_temp_run2/Planck_cutouts_T_nw_masked_rands_WFv2_COMMANDER_noINP_12arcmin_ps_part%s.npy' %(i), allow_pickle= True)
  m_temp[i] = np.load('./merged_rand_parts_nw/rands_mask_0p4medwt_6mJy150ghzv2_12arcmin_ps_part%s.npy' %(i), allow_pickle= True)
t_merged=np.concatenate((np.asarray(t_temp[0]),np.asarray(t_temp[1]),np.asarray(t_temp[2]),np.asarray(t_temp[3]),np.asarray(t_temp[4]),np.asarray(t_temp[5]),np.asarray(t_temp[6]),np.asarray(t_temp[7]),np.asarray(t_temp[8]),np.asarray(t_temp[9])),axis=0)
tp_merged=np.concatenate((np.asarray(tp_temp[0]),np.asarray(tp_temp[1]),np.asarray(tp_temp[2]),np.asarray(tp_temp[3]),np.asarray(tp_temp[4]),np.asarray(tp_temp[5]),np.asarray(tp_temp[6]),np.asarray(tp_temp[7]),np.asarray(tp_temp[8]),np.asarray(tp_temp[9])),axis=0)
mask=np.concatenate((np.asarray(m_temp[0]),np.asarray(m_temp[1]),np.asarray(m_temp[2]),np.asarray(m_temp[3]),np.asarray(m_temp[4]),np.asarray(m_temp[5]),np.asarray(m_temp[6]),np.asarray(m_temp[7]),np.asarray(m_temp[8]),np.asarray(m_temp[9])),axis=0)
t=np.asarray(t_merged)*1000
tp=np.asarray(tp_merged)*1000000
print(len(t),len(tp))
...
parser.add_argument('-clusters_or_randoms', dest='clusters_or_randoms', action='store', help='clusters_or_randoms', type=str, default='randoms')
...
if clusters_or_randoms == 'randoms':
    ...
    #sim_dic[sim_type].pop('grad_orien')
sim_dic['param_dict']=param_dict

#np.save('./randoms_m200cut_T_150GHz_healpix_nw_apod_zeros_masked_inpaint_v4_planckgrad.npy', sim_dic)
with open('./randoms_m200cut_T_150GHz_healpix_nw_apod_zeros_masked_inpaint_v5_planckgrad_WFv2_COMMANDER_noINP_12arcmin_ps_p0.pkl','wb') as f:
  pkl.dump(sim_dic, f)
sys.exit()
