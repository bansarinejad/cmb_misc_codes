...
#parser.add_argument('-dataset_fname', dest='dataset_fname', action='store', help='dataset_fname', type=str, default='../results/nx240_dx0.5/beam1.2/noise3.2/10amcutouts/m200mean_2.3e+14/5500clusters/withgaussianfg_withclustertsz_withclusterksz/TQU/clusters_5500sims0to25-1.npy')
#parser.add_argument('-dataset_fname', dest='dataset_fname', action='store', help='dataset_fname', type=str, default='../results/nx240_dx0.5/beam_1.2/noise3.2/10amcutouts/m200mean_2.3e+14/1100clusters/withgaussianfg_withclustertsz_withclusterksz/TQU/clusters_TQU_modrandomseed.npy')
#parser.add_argument('-minM', dest='minM', action='store', help='minM', type=float, default=0.0)
parser.add_argument('-massM', dest='massM', action='store', help='massM', type=float)#, default=0.0)
#parser.add_argument('-maxM', dest='maxM', action='store', help='maxM', type=float, default=4.0)
...
parser.add_argument('-random_seed_for_models', dest='random_seed_for_models', action='store', help='random_seed_for_models', type=int, default=67)
...
hdul = fits.open('../python/data/y3_gold_z_m200.fit')
data = hdul[1].data
photoz = data['Z_LAMBDA']
#mass200 = data['M200']
...
cluster_z = photoz #param_dict['cluster_z']
...
if not pol:
    ...
    tqulen=3
tqu_tit_arr=['T', 'Q', 'U']
el_temp, cl_temp = tools.get_cmb_cls(cls_file, pol = pol)
cl=cl_temp[:, 18:13000] #here cl ranges from ell=2-19900 so the indeces are 1 lower than the el/bl/nl cuts below)
el=el_temp[18:13000] 
print(len(el),len(cl))
########################
...
if make_2d:
    ...
    el_bl=np.arange(len(bl))
    bl=flatsky.cl_to_cl2d(el_bl, bl, mapparams) 
#if ilc_file is None:
#    nl_dic = tools.get_nl_dic(noiseval, el, pol = pol)
#else:
#ilc_dic = np.load(ilc_file, allow_pickle = True).item()
#cl_residual_arr=T_ilc_residual
...
nl_dic = {}
T_ilc_residual = np.load('./debug/T_95_150_220_winterfield_fullsky_cl_residual.npy', allow_pickle = True).squeeze()
nl_dic['T'] = T_ilc_residual
Q_ilc_residual = np.load('./debug/Q_95_150_220_winterfield_fullsky_cl_residual.npy', allow_pickle = True).squeeze()
nl_dic['Q'] = Q_ilc_residual
U_ilc_residual = np.load('./debug/U_95_150_220_winterfield_fullsky_cl_residual.npy', allow_pickle = True).squeeze()
nl_dic['U'] = U_ilc_residual
...
cluster_mass_arr = np.array((massM/10) * 1e14)
cluster_z_arr = np.tile( cluster_z, cluster_mass_arr.size )

###for (cluster_mass, cluster_z) in zip(cluster_mass_arr, cluster_z_arr):

keyname = (round(cluster_mass_arr/1e14, 3), 0.500)#round(cluster_z, 3))
print('\t###############')
...
M200c_list = np.tile(cluster_mass_arr, total_clusters)
...
###random_seed_arr=[3,47,139,151,181,233,283,349,373,401,433,491,571,617,647,653,677,733,769,811,907,937,947,971,991]
for simcntr in tqdm(range( totiters_for_model )):
    print('simcntr:',simcntr)

    if random_seed_for_models != -1:
        randomseedval = random_seed_for_models * simcntr
        ###randomseedval=random_seed_arr[simcntr]
    ...
    #for i in range(nclustersorrandoms):
        ...
        #     #ignore_noise_map=np.asarray( [flatsky.make_gaussian_realisation(mapparams, el, nl_dic['T'])] )
        #     if fg_gaussian:
        ...
        el_nl=np.arange(len(nl_dic['T']))
        make_2d=True
        if make_2d:
            assert mapparams is not None
            nltwod_T=flatsky.cl_to_cl2d(el_nl, nl_dic['T'], mapparams)
            nltwod_Q=flatsky.cl_to_cl2d(el_nl, nl_dic['Q'], mapparams) 
            nltwod_U=flatsky.cl_to_cl2d(el_nl, nl_dic['U'], mapparams)  
        noise_map_T=flatsky.make_gaussian_realisation(mapparams, el_nl, nl_dic['T'])#, cltwod=nltwod_T)
        noise_map_Q=flatsky.make_gaussian_realisation(mapparams, el_nl, nl_dic['Q'])#, cltwod=nltwod_Q)
        noise_map_U=flatsky.make_gaussian_realisation(mapparams, el_nl, nl_dic['U'])#, cltwod=nltwod_U)
        noise_map=np.asarray( [noise_map_T, noise_map_Q, noise_map_U] )
            #commenting out as residuals from ILC map contain both noise & foregrounds
            # if fg_gaussian: 
        ...
        # NEW: adding transfer function
        lx, ly = flatsky.get_lxly(mapparams)
        #ell=np.sqrt(lx**2 + ly**2.)
        l1, l2 = 300., 13000.
        transfer_funct = np.exp( -(l1/lx)**6. ) * np.exp( -(lx/l2)**6. )
        cmb_map = np.fft.ifft2( np.fft.fft2(cmb_map) * transfer_funct ).real
        sim_map = np.fft.ifft2( np.fft.fft2(sim_map) * transfer_funct ).real
    ...
    for simcntr in range( totiters_for_model ):
        ...
        if apply_wiener_filter:
            if pol:
                ...
                cl_noise_arr=[nl_dic['T'], nl_dic['Q'], nl_dic['U']]
            else:
                cl_signal_arr=[cl[0]]
        ...
        ##grad_mag_arr, grad_orien_arr, cutouts_rotated_arr = tools.get_rotated_tqu_cutouts(sim_arr, sim_arr, nclustersorrandoms, tqulen, mapparams, cutout_size_am, perform_rotation = False, apply_wiener_filter=apply_wiener_filter, cl_signal = cl_signal_arr, cl_noise = cl_noise_arr, lpf_gradient_filter = lpf_gradient_filter, cutout_size_am_for_grad = cutout_size_am_for_grad)
...
mdef = 'm%s%s' %(param_dict['delta'], param_dict['rho_def'])#, param_dict['cluster_mass'])
...
extrastr = '_datablnl_seed%s_mass%.3f_z%.3f' %(random_seed_for_models, keyname[0], keyname[1])
...
#np.save('./models_TQU_desnz/model%s.npy' %(extrastr), model_dic)
logline = '\t\tResults dumped in %s' %(op_fname)
print(logline)
sys.exit()
########################

#behzad commented out the block below as it wasnt being used due to sys.exit above. (06/03/23)
