...
###        print('\t\tmock dataset %s of %s' %(simcntr+1, end-start))
    ...
    print(dx)
    grad_mag_arr, grad_orien_arr, cutouts_rotated_arr, grad_orien_full_arr = tools_data_gradtest.get_rotated_tqu_cutouts(sim_arr, sim_arrp, nclustersorrandoms, tqulen, mapparams, mapparamsp, cutout_size_am, apply_wiener_filter=False, cl_signal = None, cl_noise = None, lpf_gradient_filter = None, cutout_size_am_for_grad = cutout_size_am_for_grad)
...
with open('./stacks/clusters_m200cut_T_ILC_healpix_nw_apod_zeros_masked_inpaint_planckgrad_WFv3_COMMANDER_v2_org_Behzad_fullorient_rot_test.pkl','wb') as f:
  pkl.dump(sim_dic, f)
sys.exit()
